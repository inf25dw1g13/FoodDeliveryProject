...
export class IngredientesController {
  ...
  @response(204, {
    ...
    content: {
      'application/json': {
        schema: getModelSchemaRef(Ingredientes, {partial: true, exclude: ['id']}),
      },
    },
  })
  async updateById(
    @param.path.number('id') id: number,
    @requestBody({
      description: 'Partial update of Ingredientes (all fields optional)',
      content: {
        'application/json': {
          schema: {
            type: 'object',
    ...
    const updateData: Partial<Ingredientes> = {};
    if (ingredientes) {
      for (const key in ingredientes) {
        if (ingredientes[key] !== undefined && ingredientes[key] !== null) {
          updateData[key as keyof Ingredientes] = ingredientes[key];
        }
      }
    }
    await this.ingredientesRepository.updateAll(updateData, {id: id});
  }
