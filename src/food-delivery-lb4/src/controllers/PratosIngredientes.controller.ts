...
export class PratosIngredientesController {
  ...
  async delete(
    ...
    // Se não houver filtro where, retornar erro (evita deletar todos os ingredientes acidentalmente)
    if (!where || Object.keys(where).length === 0) {
      return {count: 0};
    }
    
    // Buscar os ingredientes do prato que correspondem ao filtro where
    const ingredientes = await this.pratosRepository.ingredientes(id).find({where});
    
    // Obter os IDs dos ingredientes encontrados
    ...
    // Deletar os registros na tabela intermediária pratos_ingredientes
    // usando os IDs dos ingredientes encontrados
    // Nota: O modelo usa pratosId e ingredientesId (camelCase), mas o banco usa prato_id e ingrediente_id
    // O LoopBack faz a conversão automaticamente
