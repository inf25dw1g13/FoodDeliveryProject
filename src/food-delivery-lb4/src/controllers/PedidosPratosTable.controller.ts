...
export class PedidosPratosTableController {
  ...
  @response(204, {
    ...
    content: {
      'application/json': {
        schema: getModelSchemaRef(PedidosPratos, {partial: true, exclude: ['id']}),
      },
    },
  })
  async updateById(
    @param.path.number('id') id: number,
    @requestBody({
      description: 'Partial update of PedidosPratos (all fields optional)',
      content: {
        'application/json': {
          schema: {
            type: 'object',
    ...
    const updateData: Partial<PedidosPratos> = {};
    if (pedidosPratos) {
      for (const key in pedidosPratos) {
        if (pedidosPratos[key] !== undefined && pedidosPratos[key] !== null) {
          updateData[key as keyof PedidosPratos] = pedidosPratos[key];
        }
      }
    }
    await this.pedidosPratosRepository.updateAll(updateData, {id: id});
  }
