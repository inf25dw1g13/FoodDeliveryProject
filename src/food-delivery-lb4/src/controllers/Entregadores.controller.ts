...
export class EntregadoresController {
  ...
  @response(204, {
    ...
    content: {
      'application/json': {
        schema: getModelSchemaRef(Entregadores, {partial: true, exclude: ['id']}),
      },
    },
  })
  async updateById(
    @param.path.number('id') id: number,
    @requestBody({
      description: 'Partial update of Entregadores (all fields optional)',
      content: {
        'application/json': {
          schema: {
            type: 'object',
    ...
    const updateData: Partial<Entregadores> = {};
    if (entregadores) {
      for (const key in entregadores) {
        if (entregadores[key] !== undefined && entregadores[key] !== null) {
          updateData[key as keyof Entregadores] = entregadores[key];
        }
      }
    }
    await this.entregadoresRepository.updateAll(updateData, {id: id});
  }
  ...
  async updateEstado(
    ...
    await this.entregadoresRepository.updateAll({estado: body?.estado}, {id: id});
  }
}
