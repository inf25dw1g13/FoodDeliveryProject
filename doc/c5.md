# Capítulo 5: Base de Dados - ACID, Audit e Replicação
...
O projeto implementa testes automatizados em Python (`test_acid.py`) que validam cada uma das propriedades ACID com **4 cenários de teste por propriedade**, totalizando **16 testes**.
...
2. **Biblioteca mysql-connector-python** instalada:
   ...
   ```
3. **Container Docker do MariaDB Master** a correr:
   ```bash
   docker ps | grep food_delivery_master
   # Deve mostrar: food_delivery_master Up ... (healthy)
   ```
4. **Base de dados `food_delivery`** criada e populada (acontece automaticamente quando o container inicia pela primeira vez)
