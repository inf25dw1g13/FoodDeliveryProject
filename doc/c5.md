...
Os relatórios individuais sobre esta componente do projeto estão presentes em: [CBD_relatorios_individuais](/doc/CBD_relatorios_individuais)
...
O projeto implementa também, testes em Python [`test_acid.py`](/extra/test_acid.py) que validam cada uma das propriedades ACID com **4 cenários de teste por propriedade**, totalizando **16 testes**.
...
docker exec -it food_delivery_master mariadb -u api_user -papi_password food_delivery -e "SELECT * FROM view_audit_log LIMIT 10;"

# Fazer uma alteração e verificar
docker exec -it food_delivery_master mariadb -u api_user -papi_password food_delivery -e "UPDATE restaurantes SET nome='Restaurante Auditado' WHERE id=1;"

docker exec -it food_delivery_master mariadb -u api_user -papi_password food_delivery -e "SELECT * FROM view_audit_log WHERE tabela_nome='restaurantes' ORDER BY data_hora DESC LIMIT 1;"

# Verificar operações recentes 
docker exec -it food_delivery_master mariadb -u api_user -papi_password food_delivery -e "SELECT id, tabela_nome, operacao, data_hora FROM view_audit_log ORDER BY data_hora DESC LIMIT 10;"
```
...
A replicação MariaDB funciona através de um processo assíncrono baseado em logs binários (binary logs):
...
**Comandos para ver status das bases de dados:**

```bash
# Ver status do Master
docker exec -it food_delivery_master mariadb -u api_user -papi_password food_delivery -e "SHOW MASTER STATUS;"

# Ver status de uma réplica
docker exec -it food_delivery_master mariadb -u api_user -papi_password food_delivery -e "SHOW SLAVE STATUS\G"
```
...
docker logs food_delivery_maxscale
```
...
A arquitetura deste Projeto é adequada para ambientes de produção, garantindo que o sistema continue a funcionar mesmo em caso de falhas e permitindo escalar horizontalmente conforme a carga desejada. Mas para bases de dados reais, seria recomendado um método de replicação síncrono para evitar perdas de dados.   
